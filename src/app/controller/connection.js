...
module.exports = {
    ...
    try {
      if (req.user.type === 'ADMIN') {
        const notifications = await Connection.find({})
          .populate({
            path: 'travelPlan packagePlan travellerid packagerid',
            select: "-password"
  ...
  getConnectionByplan: async (req, res) => {
    try {
      ...
      if (req && req.user && req.user.type == "ADMIN") {
        ...
        const notifications = await Connection.find(cond)
          .populate(
            "packagerid travellerid travelPlan packagePlan",
            "-password"
          )
          .sort({ createdAt: -1 });
        res.status(200).json({
  ...
  conctionAcceptReject: async (req, res) => {
    try {
      ...
      if (payload.status === "ACCEPTED") {
        const noti = `${user.fullName} has approved to deliver ${notifications.packagePlan.name} to ${notifications.travelPlan.toaddress} before ${moment(notifications.packagePlan.delivery_date).format('DD/MM/YYYY hh:mm A')} to earn ${notifications.packagePlan.total}`
      ...
      if (payload.status === "PICUPED") {
        ...
        const content = `${notifications.packagerid.fullName} has verified pick up of ${notifications.packagePlan.name}. Deliver it within time to ${moment(notifications.packagePlan.delivery_date).format('DD/MM/YYYY hh:mm A')} to earn bonus of ${notifications.packagePlan.total}`
        await Notification.create({
          userType: req.user.type,
          senderId: req.user.id,
          receverId: notifications.travellerid._id,
      ...
      if (payload.status === "DELIVERED") {
        ...
        const content = `${notifications.packagerid.fullName} has confirmed delivery of ${notifications.packagePlan.name} to Package ${notifications.packagePlan.delivery_address}" You have earned ${notifications.packagePlan.total}`
        ...
        await notify({ to: notifications.travellerid._id, content: `Congrats! you have earned ${notifications.packagePlan.total}`, data: { type: 'status', status: 'DELIVERED' } })
  ...
  getConnectionByNoti: async (req, res) => {
    try {
      const notifications = await Connection.findById(
        ...
        .populate("packagerid travellerid travelPlan packagePlan", "-password")
        .sort({ createdAt: -1 });
  ...
  checkuserConnection: async (req, res) => {
    try {
      ...
      const travellerplan = await TravelPlan.find({ user: req.user.id, estimate_time: { $gte: new Date() } })
      const packageplan = await PackagePlan.find({ user: req.user.id, jobStatus: { $nin: ["PENDING", "REJECTED", "DELIVERED"] } })
      ...
      if (plans.length > 0) {
        ...
        await User.findByIdAndUpdate(eq.user.id, {
          fullName: '',
          email: '',
          phone: '',
          code: '',
