...
module.exports = {
  createNotification: async (req, res) => {
    try {
      ...
      let content = `${notifications.travellerid.fullName} is trying to connect for delivering ${notifications.packagePlan.name} to ${notifications.packagePlan.delivery_address}`;
    ...
    try {
      ...
      if (req && req.user && req.user.type == "ADMIN") {
        ...
        const notifications = await Notification.find({
          ...
          .populate("senderId travelPlan packagePlan", "-password")
          .sort({ createdAt: -1 });
        res.status(200).json({
          success: true,
          message: "Fetched all notification successfully",
