...
module.exports = {
            ...
            jobs1.map(async j => {
              if (f._id.toString() === j._id.toString()) {
                ...
                let notObj = {
                  ...
                  travelPlan: j._id,
                  packagePlan: d._id,
                  notification: `New delivery request : Earn ${d.total} by delivering ${d.name} to "${d.delivery_address}" for ${userDetail.fullName}`
                };
                ...
                let content = `${traveller.fullName} is heading to "${d.delivery_address}" at ${moment(j.jurney_date).format('DD/MM/YYYY hh:mm A')}`
                const notObj2 = {
                  userType: 'TRAVELLER',
                  senderId: j.user,
                ...
                let content2 = `${traveller.fullName} is leaving to ${j.delivery_address} at ${moment(d.jurney_date).format('DD/MM/YYYY hh:mm A')}`
                const notObj3 = {
                  userType: 'TRAVELLER',
                  senderId: j.user,
                  receverId: req.user.id,
  ...
  getPackagesByID: async (req, res) => {
    try {
      ...
      if (req.query.push && req.query.push === 'true') {
        const connect = await Connection.findOne({ packagePlan: req?.params.id, $and: [{ jobStatus: { $ne: "PENDING" } }, { jobStatus: { $ne: 'ACCEPTED' } }, { jobStatus: { $ne: 'ACCEPT' } }, { jobStatus: { $ne: "REJECTED" } },] }, 'travellerid')
        await notify({
          content: `"Sadanam Kayyilundo?" "Is the package with you?"`,
          to: connect.travellerid,
        });
      }
  ...
  cancelPlan: async (req, res) => {
    ...
    try {
      ...
      if (connection.length === 0) {
        packager.wallet = packager.wallet + package.total
        packager.paymetStatus = 'Pending'
        await packager.save()
